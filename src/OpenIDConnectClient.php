...
class OpenIDConnectClient
{
    ...
    public function signOut($accessToken, $redirect)
    {
        ...
        if ($redirect == null) {
            ...
            $signout_params = array('id_token_hint' => $accessToken, 'post_logout_redirect_uri' => $redirect);
        }
    ...
    public function getRedirectURL()
    {
        ...
        $host = @(($tmp = explode(':', $_SERVER['HTTP_HOST'])) ? $tmp[0] : $tmp[0]) ?: @$_SERVER['SERVER_NAME'] ?: @$_SERVER['SERVER_ADDR'];
    ...
    public function refreshToken($refresh_token)
    {
        ...
        $token_params = array('grant_type' => $grant_type, 'refresh_token' => $refresh_token, 'client_id' => $this->clientID, 'client_secret' => $this->clientSecret);
        // Convert token params to string format
        $token_params = http_build_query($token_params, null, '&');
...
' . '</RSAKeyValue>';
    ...
    private function verifyJWTclaims($claims, $accessToken = null)
    {
        ...
        return ($claims->iss == $this->getIssuer() || $claims->iss == $this->getWellKnownIssuer() || $claims->iss == $this->getWellKnownIssuer(true)) && ($claims->aud == $this->clientID || in_array($this->clientID, $claims->aud)) && $claims->nonce == $this->getNonce() && (!isset($claims->exp) || $claims->exp >= time() - $this->leeway) && (!isset($claims->nbf) || $claims->nbf <= time() + $this->leeway) && (!isset($claims->at_hash) || $claims->at_hash == $expecte_at_hash);
    }
    /**
     * @param string $str
     * @return string
